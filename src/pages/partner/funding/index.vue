    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex items-center space-x-3">
        ...
        <button
          ...
          class="inline-flex items-center px-4 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors"
        >
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group">
        ...
        <h3 class="text-2xl font-bold text-slate-900">
          ₦{{ (walletStats.planType === 'postpaid' ? Math.abs(walletStats.balance) : walletStats.balance)?.toLocaleString() || 0 }}
        </h3>
        <p v-if="walletStats.planType !== 'postpaid'" class="mt-2 text-xs font-medium" :class="walletStats.netChange >= 0 ? 'text-emerald-600' : 'text-rose-600'">
...
const fetchStats = async () => {
  try {
    const data = await PartnerPortalService.getWalletStats('operating');
...
const fetchTransactions = async (page = 1) => {
  try {
    ...
    walletStats.value.planType = response.planType || walletStats.value.planType;
  } catch (error) {
